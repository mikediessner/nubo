...
from nubo.optimisation import autograd_minimise, gen_candidates, autograd_multi_start
...
for iter in range(iters):
    ...
    # print("Noise variance: ", mll.likelihood.noise)
    
    # specify acquisition function
    # acq = MCUpperConfidenceBound(5, gp, beta=1.0, base_samples=False)
    acq = MCExpectedImprovement(mc_samples, gp, torch.max(y_train))
    
    # optimise acquisition function with numpy/scipy
    opt_bounds = torch.Tensor([[0.]*dims, [1.]*dims])
    candidates = gen_candidates(acq, dims=dims, bounds=opt_bounds, num_candidates=5, num_samples=10)
    acq.z = None
